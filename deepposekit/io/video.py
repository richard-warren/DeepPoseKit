...
class VideoReader(cv2.VideoCapture, Sequence): 
    ...
    def read(self):
        ...
        if ret:
            ...
            if self.frame_size is not None:
                frame_resized = np.zeros((self.frame_size[0], self.frame_size[1],3), dtype='uint8')
                x = min(frame.shape[1], self.frame_size[1])
                y = min(frame.shape[0], self.frame_size[0])
                frame_resized[:y,:x] = frame[:y,:x]
                frame = frame_resized
